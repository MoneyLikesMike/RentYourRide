...
export default function GuestRentalAgreementScreen({ navigation, route }) {
  ...
  const guestPhotosDisplayed = isCheckoutFlow ? guestPhotosCheckout : guestPhotosCheckIn;

  useEffect(() => {
    if (!booking) return;
    const next = isCheckoutFlow
      ? (booking.guestCheckoutDamageNotes || '').trim()
      : (booking.guestCheckInDamageNotes || booking.rentalAgreementDamageNotes || '').trim();
    setDamageNotes(next);
  }, [booking?.id, isCheckoutFlow, booking?.guestCheckoutDamageNotes, booking?.guestCheckInDamageNotes, booking?.rentalAgreementDamageNotes]);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <PictureDocumentationPlaceholderGrid
          photos={photoTab === 'guest' ? guestPhotosDisplayed : []}
        />
        ...
        <CheckRow
          ...
          label={
            'The vehicle has been throughly inspected and \nwashed and is in a good condition for rental.'
          }
