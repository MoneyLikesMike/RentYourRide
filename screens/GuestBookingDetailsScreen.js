...
export default function GuestBookingDetailsScreen({ navigation, route }) {
  const insets = useSafeAreaInsets();
  const { bookingId } = route.params || {};
  const { getBookingById, cancelGuestBooking } = useGuestBookings();
  ...
  const grandTotal = Number(
    p.grandTotal != null && p.grandTotal !== '' ? p.grandTotal : subtotal + tripFee
  );
  ...
  const openHostProfile = () => {
    navigateToUserProfile(navigation, {
      profileUser: {
        userId: booking.hostUserId || null,
        displayName: hostName,
        photoUri: ls.hostPhotoUri || null,
      },
      highlightListings: (() => {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        <TouchableOpacity style={styles.rowPair} onPress={openHostProfile} activeOpacity={0.7}>
      ...
      <View style={[styles.footer, { paddingBottom: 12 + insets.bottom }]}>
        ...
        <TouchableOpacity onPress={() => setCancelModalVisible(true)} activeOpacity={0.7}>
          <Text style={styles.cancelLink}>Cancel trip</Text>
        </TouchableOpacity>
      </View>
