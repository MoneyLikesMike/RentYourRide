...
export default function GuestConditionPhotoReviewScreen({ navigation, route }) {
  ...
  const onComplete = useCallback(() => {
    if (bookingId && photos.length) {
      updateGuestBooking(bookingId, checkoutFlow ? { guestCheckoutConditionPhotos: photos } : { checkInConditionPhotos: photos });
    }
    navigation.pop(3);
