...
import { getHostNetEarnings } from '../utils/hostBookingEarnings';
...
export default function HostRentalRequestCard({ booking, onPress }) {
  ...
  const bookedEarnings = useMemo(() => getHostNetEarnings(booking), [booking]);
  const guestPhoto = booking.guestPhotoUri
    ? { uri: String(booking.guestPhotoUri) }
    : require('../assets/icons/shape.png');

  const status = useMemo(() => getTripCardStatus(booking), [booking]);
  const primaryAction = useMemo(
    () => getTripCardPrimaryAction(status.key, { isHost: true, booking }),
    [status.key, booking]
  );

  const onCheckIn = () => {
    navigation.navigate('HostCheckInScreen', { bookingId: booking.id });
  };

  const onCheckout = () => {
    navigation.navigate('HostCheckoutScreen', { bookingId: booking.id });
  };
        ...
        <View style={styles.headerRow}>
          <TouchableOpacity style={styles.headerMain} onPress={onPress} activeOpacity={0.88}>
            ...
            <View style={styles.nameCol}>
              <View
                style={[styles.guestNameFrame, isPendingHostApproval && styles.guestNameFrameNoStatusInset]}
              >
              ...
              <Text style={styles.wantVehicleLine} numberOfLines={3}>
                <Text style={styles.wantRent}>WANTS TO RENT YOUR </Text>
        ...
        <TouchableOpacity onPress={onPress} activeOpacity={0.88}>
          ...
          <View style={styles.pillTeal}>
            <View style={styles.pillTealRow}>
              <Text style={styles.pillTealLabel}>Booked earnings:</Text>
              <Text style={styles.pillTealAmount}>${Number(bookedEarnings).toFixed(0)} CAD</Text>
            </View>
          </View>
        </TouchableOpacity>
